...
def setup():
    ...
    grid_view = Canvas(root, width=ch11_model.width * cell_size,
                       height=ch11_model.height * cell_size,
                       borderwidth=0,
    ...
    choice = StringVar(root)  # variable that holding the choice
    ...
    #
    #         Note the sticky parameter tells the layout manager to keep the buttons, essentially,
    #         stuck to the west (left) and east (right) sides, instead of being centered. This just
    #         helps the buttons stay in a good position for different window sizes.
...
def grid_handler(event):
    ...
    if ch11_model.grid_model[x][y] == 1:
        ch11_model.grid_model[x][y] = 0
        draw_cell(x, y, 'white')
    else:
        ch11_model.grid_model[x][y] = 1
        draw_cell(x, y, 'black')
...
def option_handler(event):
    ...
    if selection == 'glider':
        ch11_model.load_pattern(ch11_model.glider_pattern, 10, 10)

    elif selection == 'glider gun':
        ch11_model.load_pattern(ch11_model.glider_gun_pattern, 10, 10)

    elif selection == 'random':
        ch11_model.randomize(ch11_model.grid_model, ch11_model.width, ch11_model.height)
...
def clear_handler(event):
    ...
    is_running = False
    for i in range(0, ch11_model.height):
        for j in range(0, ch11_model.width):
            ch11_model.grid_model[i][j] = 0
...
def update():
    ...
    for i in range(0, ch11_model.height):
        for j in range(0, ch11_model.width):
            if ch11_model.grid_model[i][j] == 1:
                draw_cell(i, j, 'black')
